...
namespace yidascan {
    public partial class FrmMain : Form {
        ...
        private void ACAreaFinishTask() {
            ...
            opcParam.InitACAreaFinish(opcclient);
        ...
        private bool NotifyWeigh(string code, bool handwork = true) {
            try {
                ...
                if (re["ERPState"] == "OK") {
                    ...
                    ERPAlarm(opcScan, opcParam, ERPAlarmNo.COMMUNICATION_ERROR);
                    return false;
                }
            } catch (Exception ex) {
                ...
                try {
                    ...
                    if (str["ERPState"] == "OK") {
                        if (res.Rows.Count > 0 && res.Rows[0]["LOCATION"].ToString() != "Fail") {
                            ...
                            ShowWarning("取交地失败");
                            ERPAlarm(opcScan, opcParam, ERPAlarmNo.TO_LOCATION_ERROR);
